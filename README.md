...
| 항목  | 설명                                  | 프로젝트 사용 예시                      |
| --- | ----------------------------------- | ------------------------------- |
| 변수  | 데이터를 저장하고 사용하기 위해 이름을 지정합니다.        | `score`, `answer`, `hint_count` |
| 자료형 | 데이터의 종류에 따라 저장하고 처리하는 방법이 달라집니다.    | `int`, `str`, `dict`, `list`    |
| 조건문 | 조건에 따라 실행할 코드를 선택합니다.               | `if`, `elif`, `else`            |
| 반복문 | 특정 코드를 조건이나 데이터 개수에 따라 반복합니다.       | `for`, `while`                  |
| 함수  | 반복되는 작업을 정의하고 매개변수를 전달받아 결과를 반환합니다. | 함수 정의, 매개변수, `return`           |
